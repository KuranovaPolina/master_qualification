...
if __name__ == "__main__":
    ...
    metrics = calculate_metrics(np.array(distances_by_MVDepthNet), np.array(real_distances))
    print("MVDepthNet:", metrics)

    print("\n\t\t\tAbsRel\t\tRMSE\t\tRMSE_log\t\tSqRel\t\tAccurancy")
    metrics_by_dist = calculate_metrics_by_dist(np.array(distances_by_MVDepthNet), np.array(real_distances))
    print("MVDepthNet:", json.dumps(metrics_by_dist, indent=4, ensure_ascii=False, sort_keys=True))
    draw_metrics(metrics_by_dist)

    print("\n\t\t\tAbsRel\t\tRMSE\t\tRMSE_log\t\tSqRel\t\tAccurancy")
    metrics_by_lum = calculate_metrics_by_luminosity(np.array(distances_by_MVDepthNet), np.array(real_distances), np.array(luminosities))
    print("MVDepthNet:", metrics_by_lum)

    print()

    print("\t\t\tRuntime")
    runtime, fps = get_runtime(np.array(runtimes))
    print("MVDepthNet:", (runtime, fps))
    ...
    with open("metrics/exp6.json", "w", encoding="utf-8") as f:
